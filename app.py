...
def student_lesson_page(lesson_id: int):
    ...
    <script>
      ...
      document.getElementById("nextSlideBtn").addEventListener("click", async ()=>{{ if (currentIndex < slides.length - 1) {{ await saveProgress(); currentIndex++; renderSlide(); return; }} const data = await saveProgress(true); const panel = document.getElementById("nextLessonPanel"); if (panel && data) {{ panel.style.display = "block"; panel.innerHTML = `<div class='xp-panel' style='display:block'><h3 style='margin:0 0 6px;'>🎉 Lesson Completed</h3><p style='margin:0 0 8px;'>+${{data.xp_earned || 15}} XP Earned</p><p style='margin:0 0 10px;'>${{data.next_lesson_title || ""}}</p></div>`; }} if (data && data.redirect_url) setTimeout(()=>window.location.href=data.redirect_url, 1200); }});
