    ...
    for s in slides:
        ...
        if s.activity_json:
            try:
                ...
                activity_payload = None
        slide_payload.append({"id": s.id, "slide_order":s.slide_order, "slide_type":s.slide_type, "title":(s.title_si if is_si else s.title_en) or "", "content":(s.content_si if is_si else s.content_en) or "", "image_url":s.image_url or "", "video_url":s.video_url or "", "activity":activity_payload})
