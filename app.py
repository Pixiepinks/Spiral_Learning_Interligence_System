...
def student_lesson_page(lesson_id: int):
    ...
    <script>
      const lessonId = {lesson.id}; const slides = {json.dumps(slide_payload)}; const isSinhala = {str(is_si).lower()}; let currentIndex = Math.max(0, slides.findIndex((s)=>s.slide_order === {int(progress.current_slide_order)}));
      ...
      function render_activity_slide(activityData) {{ if (!activityData || typeof activityData !== "object") return ""; const activityType = String(activityData.type || "").trim().toLowerCase(); const questionTitle = isSinhala ? (activityData.question_si || activityData.question_en || "Activity") : (activityData.question_en || activityData.question_si || "Activity"); if (activityType === "tap_correct_picture") {{ const items = Array.isArray(activityData.items) ? activityData.items : []; if (!items.length) return ""; const cards = items.map((item, idx)=>`<button type="button" class="activity-card" data-item-index="${{idx}}" data-correct="${{Boolean(item.correct)}}"><span class="activity-emoji">${{item.emoji || "🧩"}}</span><span class="activity-name">${{item.name || `Item ${{idx + 1}}`}}</span></button>`).join(""); return `<div class="activity-wrap" data-activity-type="tap_correct_picture"><h3 class="activity-question">${{questionTitle}}</h3><div class="activity-grid">${{cards}}</div><div class="activity-actions"><button type="button" class="activity-check-btn" id="checkAnswerBtn">Check Answer</button><div class="activity-result" id="activityResult" style="display:none;"></div></div></div>`; }} if (activityType === "mcq") {{ const options = Array.isArray(activityData.options) ? activityData.options : []; if (!options.length) return `<div class="activity-wrap"><h3 class="activity-question">${{questionTitle}}</h3><p>Invalid quiz configuration.</p></div>`; const optionCards = options.map((option, idx)=>{{ const label = isSinhala ? (option.text_si || option.text_en || `Option ${{idx + 1}}`) : (option.text_en || option.text_si || `Option ${{idx + 1}}`); return `<button type="button" class="activity-card mcq-option" data-option-index="${{idx}}" data-correct="${{Boolean(option.correct)}}"><span class="activity-name">${{label}}</span></button>`; }}).join(""); return `<div class="activity-wrap" data-activity-type="mcq"><h3 class="activity-question">${{questionTitle}}</h3><div class="activity-grid">${{optionCards}}</div><div class="activity-actions"><button type="button" class="activity-check-btn" id="checkAnswerBtn">Check Answer</button><div class="activity-result" id="activityResult" style="display:none;" data-success-msg="${{isSinhala ? (activityData.success_message_si || activityData.success_message_en || "Great job!") : (activityData.success_message_en || activityData.success_message_si || "Great job!")}}" data-failure-msg="${{isSinhala ? (activityData.failure_message_si || activityData.failure_message_en || "Try again.") : (activityData.failure_message_en || activityData.failure_message_si || "Try again.")}}"></div></div></div>`; }} if (["drag_drop","matching","fill_blank","ordering"].includes(activityType)) return `<div class="activity-wrap"><h3 class="activity-question">${{questionTitle}}</h3><p>Activity type <strong>${{activityType}}</strong> is coming soon.</p></div>`; return ""; }}
      function wireTapCorrectPictureInteraction(mediaWrap) {{ const cards = mediaWrap.querySelectorAll(".activity-card"); const resultBox = mediaWrap.querySelector("#activityResult"); const checkBtn = mediaWrap.querySelector("#checkAnswerBtn"); cards.forEach((card) => {{ card.addEventListener("click", () => {{ card.classList.toggle("selected"); card.classList.remove("correct", "wrong"); if (resultBox) resultBox.style.display = "none"; }}); }}); if (!checkBtn) return; checkBtn.addEventListener("click", () => {{ let allGood = true; cards.forEach((card) => {{ const selected = card.classList.contains("selected"); const isCorrect = card.dataset.correct === "true"; card.classList.remove("correct", "wrong"); if (selected && isCorrect) card.classList.add("correct"); if (selected && !isCorrect) {{ card.classList.add("wrong"); allGood = false; }} if (!selected && isCorrect) allGood = false; }}); if (resultBox) {{ resultBox.style.display = "inline-block"; resultBox.className = `activity-result ${{allGood ? "success" : "fail"}}`; resultBox.textContent = allGood ? "Great Job!" : "Try Again"; }} }}); }}
      function wireMcqInteraction(mediaWrap) {{ const cards = mediaWrap.querySelectorAll(".mcq-option"); const resultBox = mediaWrap.querySelector("#activityResult"); const checkBtn = mediaWrap.querySelector("#checkAnswerBtn"); let selectedCard = null; cards.forEach((card) => {{ card.addEventListener("click", () => {{ cards.forEach((item)=>item.classList.remove("selected", "correct", "wrong")); card.classList.add("selected"); selectedCard = card; if (resultBox) resultBox.style.display = "none"; }}); }}); if (!checkBtn) return; checkBtn.addEventListener("click", () => {{ if (!selectedCard) {{ if (resultBox) {{ resultBox.style.display = "inline-block"; resultBox.className = "activity-result fail"; resultBox.textContent = isSinhala ? "පිළිතුරක් තෝරන්න." : "Please select an answer."; }} return; }} const isCorrect = selectedCard.dataset.correct === "true"; selectedCard.classList.remove("correct", "wrong"); selectedCard.classList.add(isCorrect ? "correct" : "wrong"); if (resultBox) {{ resultBox.style.display = "inline-block"; resultBox.className = `activity-result ${{isCorrect ? "success" : "fail"}}`; resultBox.textContent = isCorrect ? (resultBox.dataset.successMsg || "Great job!") : (resultBox.dataset.failureMsg || "Try again."); }} }}); }}
      function renderSlide() {{ const current = slides[currentIndex]; document.getElementById("slideTypePill").textContent = current.slide_type.replaceAll("_", " "); document.getElementById("slideTitle").textContent = current.title || "Slide"; document.getElementById("slideContent").textContent = current.content || ""; const pct = Math.round(((currentIndex + 1) / slides.length) * 100); document.getElementById("completionText").textContent = `Completion: ${{pct}}%`; document.getElementById("completionBar").style.width = `${{pct}}%`; const mediaWrap = document.getElementById("slideMediaWrap"); mediaWrap.innerHTML = ""; if (current.slide_type === "intro_video" && current.video_url) {{ const iframe = document.createElement("iframe"); iframe.className = "slide-video"; iframe.src = normalizeYouTube(current.video_url); iframe.allowFullscreen = true; mediaWrap.appendChild(iframe); }} else if (current.image_url) {{ const image = document.createElement("img"); image.className = "slide-media"; image.src = current.image_url; mediaWrap.appendChild(image); }} const activityHtml = render_activity_slide(current.activity); if (activityHtml) {{ mediaWrap.insertAdjacentHTML("beforeend", activityHtml); const activityType = String(current.activity?.type || "").toLowerCase(); if (activityType === "tap_correct_picture") wireTapCorrectPictureInteraction(mediaWrap); if (activityType === "mcq") wireMcqInteraction(mediaWrap); }} document.getElementById("progressDots").innerHTML = slides.map((s, i)=>`<span class="lesson-dot ${{i < currentIndex ? "completed" : ""}} ${{i === currentIndex ? "active" : ""}}"></span>`).join(""); document.getElementById("prevSlideBtn").disabled = currentIndex === 0; document.getElementById("nextSlideBtn").textContent = currentIndex === slides.length - 1 ? "Finish" : "Next"; document.getElementById("xpPanel").style.display = currentIndex === slides.length - 1 ? "block" : "none"; }}
