...
def render_drag_drop_group_container_input(question: "Question", medium_key: str = "en") -> str:
    ...
    return f"""
    <div class='drag-group-wrap drag-drop-question' data-question-id='{question.id}'>
...
def drag_drop_group_assets() -> str:
    return """
    <style>.drag-drop-question{max-width:720px}.drag-items-row{display:flex !important;flex-direction:row !important;gap:16px;align-items:center;flex-wrap:wrap;margin:12px 0 20px 0}.drag-item{width:64px !important;height:64px !important;max-width:64px !important;max-height:64px !important;object-fit:contain;cursor:grab;touch-action:none;user-select:none;position:relative;z-index:20;display:flex;align-items:center;justify-content:center}.drag-item img{width:100%;height:100%;object-fit:contain;pointer-events:none}.drop-container{position:relative;width:420px;height:220px;border:2px dashed #bbb;border-radius:12px;overflow:hidden}.drop-container img{width:100%;height:100%;object-fit:contain;pointer-events:none}.drag-basket.active{box-shadow:0 0 0 3px rgba(147,197,253,.4)}.drag-item.in-basket{position:absolute;z-index:30}</style>
    <script>
    function initDragGroupUI(root=document){root.querySelectorAll('.drag-group-wrap').forEach((wrap)=>{if(wrap.dataset.ready==='1')return;wrap.dataset.ready='1';const bank=wrap.querySelector('.drag-items-row');const basket=wrap.querySelector('.drag-basket');const hidden=wrap.querySelector('.drag-answer-json') || wrap.querySelector('#interactive_drag_answer');if(!bank||!basket||!hidden)return;const clamp=(n,min,max)=>Math.min(Math.max(n,min),max);const updateHidden=()=>{const placed={};[...basket.querySelectorAll('.drag-item')].forEach((i)=>{placed[i.dataset.itemId]={x:parseFloat(i.style.left)||0,y:parseFloat(i.style.top)||0,group:i.dataset.group||'',id:i.dataset.itemId||''};});hidden.value=JSON.stringify(placed);};const placeInBank=(el)=>{bank.appendChild(el);el.classList.remove('in-basket');el.style.position='relative';el.style.left='0px';el.style.top='0px';el.style.zIndex='20';};const placeInBasket=(el,cx,cy)=>{const br=basket.getBoundingClientRect();basket.appendChild(el);el.classList.add('in-basket');el.style.position='absolute';el.style.zIndex='30';const size=64;const x=clamp(cx-br.left-size/2,0,Math.max(0,br.width-size));const y=clamp(cy-br.top-size/2,0,Math.max(0,br.height-size));el.style.left=x+'px';el.style.top=y+'px';};[...wrap.querySelectorAll('.drag-item')].forEach((el)=>{let dragging=false,dx=0,dy=0,originParent=null;el.addEventListener('pointerdown',(e)=>{dragging=true;originParent=el.parentElement;el.setPointerCapture(e.pointerId);const r=el.getBoundingClientRect();dx=e.clientX-r.left;dy=e.clientY-r.top;el.style.position='fixed';el.style.left=(e.clientX-dx)+'px';el.style.top=(e.clientY-dy)+'px';el.style.zIndex='9999';document.body.appendChild(el);e.preventDefault();});el.addEventListener('pointermove',(e)=>{if(!dragging)return;el.style.left=(e.clientX-dx)+'px';el.style.top=(e.clientY-dy)+'px';const br=basket.getBoundingClientRect();const inside=e.clientX>=br.left&&e.clientX<=br.right&&e.clientY>=br.top&&e.clientY<=br.bottom;basket.classList.toggle('active',inside);});const finishDrag=(e)=>{if(!dragging)return;dragging=false;basket.classList.remove('active');const br=basket.getBoundingClientRect();const inside=e.clientX>=br.left&&e.clientX<=br.right&&e.clientY>=br.top&&e.clientY<=br.bottom;if(inside){placeInBasket(el,e.clientX,e.clientY);}else if(originParent===basket){placeInBasket(el,e.clientX,e.clientY);}else{placeInBank(el);}updateHidden();};el.addEventListener('pointerup',finishDrag);el.addEventListener('pointercancel',()=>{dragging=false;basket.classList.remove('active');if(originParent===basket){placeInBasket(el,0,0);}else{placeInBank(el);}updateHidden();});});updateHidden();});}
...
def student_chapter_page(chapter_id: int):
    ...
    function showInteractionModal(contentId, interaction) {{
      ...
      if (qType === 'mcq') {{
        ...
        controlHtml = `<div class='drag-group-wrap drag-drop-question' data-question-id='interactive'><div class='drag-items-row'>${{items.map(it => `<div class='drag-item' data-item-id='${{escapeHtml(String(it.id||''))}}' data-group='${{escapeHtml(String(it.group||''))}}'><img src='${{escapeHtml(normalizeLocalImageUrl(it.image_url||''))}}'></div>`).join('')}}</div><div class='drop-container drag-basket' data-container='1'><img src='${{escapeHtml(basket)}}'></div><input id='interactive_drag_answer' type='hidden'></div>`;
