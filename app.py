...
def student_dashboard():
    ...
    <script>document.addEventListener("DOMContentLoaded", function () {{const changeBtn=document.getElementById("changePhotoBtn");const modal=document.getElementById("photoUploadModal");const closeBtn=document.getElementById("closePhotoModal");const startBtn=document.getElementById("startCameraBtn");const captureBtn=document.getElementById("captureBtn");const video=document.getElementById("cameraStream");const preview=document.getElementById("cameraPreview");const canvas=document.getElementById("cameraCanvas");const hiddenData=document.getElementById("cameraImageData");let stream=null;function stopCam(){{if(stream){{stream.getTracks().forEach(track=>track.stop());stream=null;}}if(video){{video.style.display="none";video.srcObject=null;}}if(captureBtn){{captureBtn.style.display="none";}}}}if(changeBtn&&modal){{changeBtn.addEventListener("click", function (e) {{e.preventDefault();modal.classList.add("open");modal.style.display="flex";}});}}if(closeBtn&&modal){{closeBtn.addEventListener("click", function () {{modal.classList.remove("open");modal.style.display="none";stopCam();}});}}if(startBtn&&video&&captureBtn){{startBtn.addEventListener("click", async function () {{if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){{alert("Camera is not supported on this device.");return;}}try{{stream=await navigator.mediaDevices.getUserMedia({{video:true}});video.srcObject=stream;video.style.display="block";captureBtn.style.display="inline-block";}}catch(error){{alert("Camera permission denied. Please use file upload.");}}}});}}if(captureBtn&&video&&canvas&&hiddenData&&preview){{captureBtn.addEventListener("click", function () {{const context=canvas.getContext("2d");canvas.width=video.videoWidth||640;canvas.height=video.videoHeight||480;context.drawImage(video,0,0,canvas.width,canvas.height);const data=canvas.toDataURL("image/webp",0.9);hiddenData.value=data;preview.src=data;preview.style.display="block";stopCam();}});}}window.addEventListener("click", function (event) {{if(event.target===modal){{modal.classList.remove("open");modal.style.display="none";stopCam();}}}});}});const progressCtx=document.getElementById('progressOverviewChart');if(progressCtx){{const practiceData={json.dumps(chart_practice_points)};const datasets=[{{label:'SkillScan',data:{json.dumps(chart_result_scores)},borderColor:'#2563eb',backgroundColor:'rgba(37,99,235,0.2)',tension:0.25,fill:false}}];if(practiceData.length){{datasets.push({{label:'Practice',data:practiceData,parsing:{{xAxisKey:'x',yAxisKey:'y'}},borderColor:'#16a34a',backgroundColor:'rgba(22,163,74,0.2)',tension:0.25,fill:false}});}}new Chart(progressCtx,{{type:'line',data:{{labels:{json.dumps(chart_labels)},datasets:datasets}},options:{{responsive:true,scales:{{x:{{title:{{display:true,text:'{text["date"]}'}}}},y:{{title:{{display:true,text:'{text["score"]}'}},min:0,max:100}}}}}});}}</script>
    </body></html>
    """
