...
def upload_image_analysis_question_image_to_supabase(question_id: int, file_storage) -> tuple[str | None, str | None]:
    if not file_storage or not file_storage.filename:
        return None, None
    ...
    object_name = f"image-analysis/{question_id}/{uuid.uuid4().hex}.webp"
    ...
    try:
        ...
        return None, f"Image upload failed: {exc.read().decode('utf-8', 'ignore') or exc.reason}"
    except URLError as exc:
        return None, f"Image upload failed: {exc.reason}"
...
def display_image_analysis_correct_answer(answer: str | None) -> str:
    """Return a student-friendly worksheet answer label."""
    value = _collapse_answer_spaces(str(answer or ""))
    if not value:
        return ""
    match = re.fullmatch(r"[A-Za-z]{1,4}\s*=\s*(.+)", value)
    if match:
        return _collapse_answer_spaces(match.group(1))
    return value


def _letter_list_answer(value: str | None) -> tuple[str, ...] | None:
    ...
    if not letters or len(letters) > 6:
        return None
    return tuple(sorted(dict.fromkeys(letters)))


def _normalized_free_text(value: str | None) -> str:
    return re.sub(r"[^\w]+", " ", str(value or "").casefold()).strip()


def image_analysis_answers_match(student_answer: str | None, accepted_answers: list[str], validation_mode: str | None = None) -> bool:
    """Worksheet matcher that keeps smart matching but accepts common student variants."""
    candidates: list[str] = []
    for answer in accepted_answers:
        for option in split_accepted_answers(answer):
            option = option.strip()
            if not option:
                continue
            candidates.append(option)
            display_answer = display_image_analysis_correct_answer(option)
            if display_answer and display_answer not in candidates:
                candidates.append(display_answer)
            if "=" in option:
                rhs = option.split("=", 1)[1].strip()
                if rhs and rhs not in candidates:
                    candidates.append(rhs)

    if any_answers_match(student_answer, candidates, validation_mode):
        return True

    student_letters = _letter_list_answer(student_answer)
    if student_letters and any(student_letters == _letter_list_answer(candidate) for candidate in candidates):
        return True

    student_text = _normalized_free_text(student_answer)
    candidate_texts = [_normalized_free_text(candidate) for candidate in candidates]
    if student_text and student_text in candidate_texts:
        return True

    # Accept the expected explanatory variants for this worksheet's final reason question.
    reason_tokens = set(student_text.split())
    if {"equal", "distance"}.issubset(reason_tokens) and "o" in reason_tokens:
        if any("equal radii" in text or "equal distance" in text for text in candidate_texts):
            return True
    if {"oa", "ob", "oc", "equal"}.issubset(reason_tokens) and ("radii" in reason_tokens or "radius" in reason_tokens):
        if any("equal radii" in text or "equal distance" in text for text in candidate_texts):
            return True
    return False
...
def evaluate_image_analysis_worksheet(question: "Question", form) -> tuple[bool, list[dict], int, int]:
    ...
    for sub_question in sub_questions:
        ...
        is_correct = bool(student_answer) and image_analysis_answers_match(student_answer_raw, accepted_answers, validation_mode)
        if is_correct:
            earned_marks += marks
        rows.append({
            "id": sub_question.id,
            ...
            "correct_answer": display_image_analysis_correct_answer(correct_answer_en),
            "correct_answer_en": display_image_analysis_correct_answer(correct_answer_en),
            "correct_answer_si": display_image_analysis_correct_answer(correct_answer_si),
...
def render_image_analysis_worksheet_input(question: "Question", medium_key: str) -> str:
    ...
    if question.image_url:
        image_html = f"""
        <div class='image-analysis-image-frame'>
          <img src='{escape(normalize_local_image_url(question.image_url))}' alt='Image analysis worksheet image' class='image-analysis-main-image'>
        </div>
        """
    rows = []
    for index, sub_question in enumerate(get_question_sub_questions(question.id), start=1):
        ...
        rows.append(f"""
        <div class='image-analysis-sub-question' data-sub-question-id='{sub_question.id}'>
          <div class='image-analysis-question-main'>
            <span class='image-analysis-number-badge'>{index}</span>
            <label class='image-analysis-question-text' for='qsub_{question.id}_{sub_question.id}'>
              {escape(question_text)}
            </label>
          </div>
          <div class='image-analysis-answer-cell'>
            <input id='qsub_{question.id}_{sub_question.id}' name='qsub_{question.id}_{sub_question.id}' type='text' inputmode='{inputmode}' autocomplete='off' placeholder='පිළිතුර ලියන්න'>
            <span class='image-analysis-result-icon' aria-live='polite'></span>
            <span class='image-analysis-feedback' aria-live='polite'></span>
          </div>
        </div>
        """)
    return f"""
...
def student_dashboard():
    ...
    <nav class="mobile-bottom-nav" aria-label="Student quick navigation"><a class="active" href="/student-dashboard">🏠<span>Home</span></a><a href="/student/subjects">📚<span>Subjects</span></a><a href="/student/live-classes">🎥<span>Live</span></a><a href="/student/assignments">📝<span>Tasks</span></a><a href="/student/ai-tutor">✨<span>AI</span></a></nav>
    <script>
(function () {{
...
def parse_question_form_data(has_uploaded_image: bool = False, existing_image_url: str = "") -> tuple[dict, str | None]:
    ...
    elif question_type == "image_analysis_worksheet" and not existing_image_url and not has_uploaded_image:
        return {}, "Image Analysis Worksheet requires a parent image upload."
...
def parse_image_analysis_sub_questions_from_form() -> tuple[list[dict], str | None]:
    ...
    for index in range(1, count + 1):
        ...
        if not question_text_en or not question_text_si or not correct_answer_en or not correct_answer_si:
            return [], f"Please complete English/Sinhala question text and correct answers for sub-question {index}."
        try:
            marks = int(marks_raw)
        except ValueError:
...
def build_manual_interim_slide_html(slide: LessonSlide, questions: list[Question], medium_key: str, is_si: bool) -> str:
    ...
    submit_label = "ලකුණු බලන්න" if is_si else "Check Marks"
    return f"""
...
def student_lesson_page(lesson_id: int):
    ...
    chapter_lessons = (
        Lesson.query.filter_by(chapter_id=chapter.id, is_active=True)
        .order_by(Lesson.lesson_order.asc(), Lesson.id.asc())
        .all()
    )
    chapter_lesson_ids = [row.id for row in chapter_lessons]
    lesson_progress_rows = (
        StudentLessonProgress.query.filter(
            StudentLessonProgress.student_id == student.id,
            StudentLessonProgress.lesson_id.in_(chapter_lesson_ids),
        ).all()
        if chapter_lesson_ids
        else []
    )
    completed_lesson_ids = {row.lesson_id for row in lesson_progress_rows if row.is_completed}

    journey_items = []
    previous_lessons_completed = True
    for journey_lesson in chapter_lessons:
        journey_is_completed = journey_lesson.id in completed_lesson_ids
        journey_is_current = journey_lesson.id == lesson.id
        journey_is_assessment = (journey_lesson.lesson_type or "").strip().lower() == "assessment"
        journey_is_unlocked = previous_lessons_completed

        if journey_is_completed:
            status_key = "completed"
            status_icon = "✓"
            status_label = "Completed" if not is_si else "සම්පූර්ණයි"
        elif not journey_is_unlocked:
            status_key = "locked"
            status_icon = "🔒"
            status_label = "Locked" if not is_si else "අගුලු දමා ඇත"
        elif journey_is_current:
            status_key = "current"
            status_icon = "▶"
            status_label = "Current lesson" if not is_si else "වත්මන් පාඩම"
        else:
            status_key = "unlocked"
            status_icon = "○"
            status_label = "Unlocked" if not is_si else "විවෘතයි"

        title_prefix = "Assessment" if (journey_is_assessment and not is_si) else ("ඇගයීම" if journey_is_assessment else f"Lesson {journey_lesson.lesson_order}")
        item_label = title_prefix if journey_is_assessment else f"{title_prefix}"
        href = url_for("student_lesson_page", lesson_id=journey_lesson.id) if status_key in {"completed", "current", "unlocked"} else "#"
        item_tag = "a" if status_key != "locked" else "span"
        item_attrs = f"href='{escape(href)}'" if item_tag == "a" else "aria-disabled='true'"
        journey_items.append(
            f"<{item_tag} class='lesson-journey-step {status_key}' {item_attrs} "
            f"aria-label='{escape(status_label)}: {escape(item_label)}'>"
            f"<span class='lesson-journey-icon' aria-hidden='true'>{status_icon}</span>"
            f"<span class='lesson-journey-label'>{escape(item_label)}</span>"
            f"<span class='lesson-journey-status'>{escape(status_label)}</span>"
            f"</{item_tag}>"
        )
        previous_lessons_completed = previous_lessons_completed and journey_is_completed

    completed_lessons_count = len(completed_lesson_ids.intersection(chapter_lesson_ids))
    total_lessons_count = len(chapter_lesson_ids)
    lessons_completed_summary = (
        f"{completed_lessons_count} of {total_lessons_count} lessons completed"
        if not is_si
        else f"පාඩම් {total_lessons_count}න් {completed_lessons_count}ක් සම්පූර්ණයි"
    )
    lesson_journey_html = "".join(journey_items)
    ...
    <script>
      ...
      function wireManualInterimTest(mediaWrap, current) {{
        const form = mediaWrap.querySelector('.manual-test-form');
        if (!form) return;
        const status = mediaWrap.querySelector('.manual-test-status');
        const resultBox = mediaWrap.querySelector('.manual-test-result');
        form.addEventListener('submit', async (event) => {{
          event.preventDefault();
          const submitBtn = form.querySelector('.manual-test-submit');
          if (submitBtn) submitBtn.disabled = true;
          if (status) status.textContent = isSinhala ? 'පරීක්ෂා කරමින්...' : 'Checking...';
          try {{
            const body = new FormData(form);
            body.append('slide_id', current.id);
            const res = await fetch('/student/lesson/' + lessonId + '/manual-interim-test', {{ method: 'POST', body }});
            const data = await res.json();
            if (!res.ok || !data.ok) {{ throw new Error(data.error || 'Could not submit test.'); }}
            const scoreText = isSinhala ? `ලකුණු: ${{data.correct_answers}}/${{data.total_questions}} (${{data.score}}%)` : `Score: ${{data.correct_answers}}/${{data.total_questions}} (${{data.score}}%)`;
            if (resultBox) {{
              resultBox.style.display = 'block';
              resultBox.innerHTML = `<div>${{escapeHtml(scoreText)}}</div><p style="margin:.35rem 0 0;font-weight:600;">${{escapeHtml(isSinhala ? 'ඔබට දැන් පාඩම ඉදිරියට ගෙන යා හැක.' : 'You can now continue the lesson.')}}</p>`;
            }}
            (data.answers || []).forEach((answer) => {{
              const article = form.querySelector(`[data-question-id="${{answer.question_id}}"]`);
              if (!article) return;
              if (answer.question_type === 'image_analysis_worksheet') {{
                let rows = [];
                try {{ rows = JSON.parse(answer.answer || '[]'); }} catch(e) {{ rows = []; }}
                rows.forEach((row) => {{
                  const card = article.querySelector(`.image-analysis-sub-question[data-sub-question-id="${{row.id}}"]`);
                  if (!card) return;
                  card.classList.remove('is-correct', 'is-wrong');
                  card.classList.add(row.is_correct ? 'is-correct' : 'is-wrong');
                  const icon = card.querySelector('.image-analysis-result-icon');
                  const feedback = card.querySelector('.image-analysis-feedback');
                  const input = card.querySelector('input');
                  const correctAnswer = String(row.correct_answer || row.correct_answer_si || row.correct_answer_en || '').trim();
                  if (icon) icon.textContent = row.is_correct ? '✅' : '❌';
                  if (feedback) {{
                    feedback.textContent = row.is_correct ? `නිවැරදියි · නිවැරදි පිළිතුර: ${{correctAnswer}}` : `නිවැරදි පිළිතුර: ${{correctAnswer}}`;
                  }}
                  if (input) input.setAttribute('aria-invalid', row.is_correct ? 'false' : 'true');
                }});
                return;
              }}
              const wrap = article.querySelector('.tap-correct-image-wrap');
              if (!wrap) return;
              let selected = [];
              let correct = [];
              try {{ selected = JSON.parse(answer.answer || '[]'); }} catch(e) {{ selected = []; }}
              try {{ correct = JSON.parse(answer.correct_answer || '[]'); }} catch(e) {{ correct = []; }}
              const selectedSet = new Set(selected.map(String));
              const correctSet = new Set(correct.map(String));
              wrap.querySelectorAll('.tap-correct-image-card').forEach((card) => {{
                const id = String(card.dataset.imageId || '');
                card.classList.remove('selected');
                if (correctSet.has(id)) card.classList.add('correct');
                else if (selectedSet.has(id)) card.classList.add('wrong');
              }});
              const feedback = wrap.querySelector('.tap-correct-image-feedback');
              if (feedback) feedback.textContent = answer.is_correct ? 'නිවැරදියි!' : 'නැවත උත්සාහ කරන්න';
            }});
            if (status) status.textContent = isSinhala ? 'සම්පූර්ණයි' : 'Submitted';
            solvedQuizSlides.add(current.id);
            enableFinishLessonButton();
            form.querySelectorAll('input,select,button,textarea').forEach((el)=>{{ if (!el.classList.contains('manual-test-submit')) el.disabled = true; }});
          }} catch (err) {{
            console.error('Manual interim test failed:', err);
            alert(err.message || (isSinhala ? 'පරීක්ෂණය සුරැකිය නොහැක.' : 'Could not save the test.'));
...
def admin_add_question():
    ...
    if question.question_type == "image_analysis_worksheet":
        uploaded_image_url, upload_error = upload_image_analysis_question_image_to_supabase(question.id, request.files.get("question_image"))
        if upload_error:
            db.session.rollback()
            return render_question_form("/admin/add-question", request.form, "Add New Question", "Save Question", upload_error), 400
        question.image_url = uploaded_image_url
...
def admin_edit_question(question_id: int):
    ...
    elif form_data["question_type"] == "image_analysis_worksheet":
        question.image_url = uploaded_image_url or question.image_url
    else:
...
def test_page() -> str:
    ...
    for q in questions:
        ...
        question_blocks.append(f"<div style='margin:16px 0;padding:12px;border:1px solid #ddd;'><p><strong>Q{q.id}.</strong> {question_text}</p>{'' if (is_tap_select_image_question(q) or is_tap_correct_image_question(q) or is_drag_drop_group_container_question(q) or is_image_analysis_worksheet_question(q)) else image_html}{answer_html}</div>")
    ...
    <html lang='{'si' if selected_medium == 'Sinhala' else 'en'}'>
      <head>
        ...
        <style>
          ...
          .image-analysis-worksheet { font-family:'Noto Sans Sinhala','Iskoola Pota','Nirmala UI',Inter,Arial,sans-serif; display:grid; gap:20px; max-width:960px; margin:0 auto; }
          .image-analysis-image-frame { display:flex; justify-content:center; padding:16px; border-radius:24px; background:linear-gradient(180deg,#fff,#f8fbff); border:1px solid #bfdbfe; box-shadow:0 18px 42px rgba(37,99,235,.10); }
          .image-analysis-main-image { display:block; width:100%; max-width:min(100%,760px); height:auto; border-radius:18px; border:1px solid #dbeafe; box-shadow:0 14px 28px rgba(15,23,42,.08); object-fit:contain; }
          .image-analysis-sub-questions { display:grid; gap:14px; }
          .image-analysis-sub-question { display:grid; grid-template-columns:minmax(220px,1fr) minmax(260px,.9fr); align-items:center; gap:16px; padding:16px; border:1px solid #bfdbfe; border-radius:18px; background:#fff; box-shadow:0 10px 28px rgba(15,23,42,.07); }
          .image-analysis-question-main { display:flex; align-items:flex-start; gap:12px; }
          .image-analysis-number-badge { display:inline-grid; place-items:center; flex:0 0 36px; width:36px; height:36px; border-radius:999px; background:linear-gradient(135deg,#2563eb,#38bdf8); color:#fff; font-size:17px; font-weight:900; }
          .image-analysis-question-text { color:#0f172a; font-size:17px; font-weight:850; line-height:1.65; }
          .image-analysis-answer-cell { display:grid; grid-template-columns:minmax(120px,1fr) auto; align-items:center; gap:10px; }
          .image-analysis-answer-cell input { width:100%; min-height:52px; box-sizing:border-box; padding:13px 15px; border:1.5px solid #bfdbfe; border-radius:14px; background:#f8fbff; color:#0f172a; font-size:18px; font-weight:800; }
          .image-analysis-result-icon { font-size:24px; }
          .image-analysis-feedback { grid-column:1/-1; font-size:15px; font-weight:900; }
          .question-image {{
            max-width: 250px;
            width: 100%;
            height: auto;
          ...
          .image-analysis-sub-question { grid-template-columns: 1fr; padding: 14px; }
          .question-image {{
              max-width: 180px;
            }}
          }}
    ...
    for q in questions:
        ...
        if is_matching_pairs_question(q):
            ...
            correct_answer = json.dumps([{
                "id": row["id"],
                "correct_answer_en": row["correct_answer_en"],
                "correct_answer_si": row["correct_answer_si"],
                "marks": row["marks"],
        ...
        if is_matching_pairs_question(q):
            ...
            for idx, row in enumerate(rows, start=1):
                ...
                parts.append(
                    ...
                    f"Correct Answer: {escape(row.get(f'correct_answer_{medium_key}') or row.get('correct_answer_en') or '')}<br>"
...
def submit_practice() -> str:
    ...
    for q in questions:
        ...
        if is_matching_pairs_question(q):
            ...
            for idx, row in enumerate(rows, start=1):
                text = row.get(f"question_text_{medium_key}") or row.get("question_text_en") or ""
                correct_text = row.get(f"correct_answer_{medium_key}") or row.get("correct_answer_en") or ""
                color = '#16a34a' if row.get('is_correct') else '#dc2626'
                parts.append(
                    f"<div style='margin-bottom:8px;'><strong>{idx}. {escape(text)}</strong><br>"
                    f"Your Answer: <span style='color:{color}'>{escape(row.get('student_answer') or t(selected_medium, 'not_answered'))}</span><br>"
                    f"Correct Answer: {escape(correct_text)}<br>"
                    f"Score: {int(row.get('earned_marks') or 0)}/{int(row.get('marks') or 0)}</div>"
                )
            student_answer_text = ''.join(parts) or t(selected_medium, 'not_answered')
            correct_answer_text = f"{sum(int(row.get('earned_marks') or 0) for row in rows)}/{sum(int(row.get('marks') or 0) for row in rows)}"
        elif is_short_answer_question(q):
            student_answer_text = student_answer or t(selected_medium, "not_answered")
            correct_answer_text = correct_answer or t(selected_medium, "not_answered")
        elif student_answer in option_label_key:
...
def student_homework_detail(homework_id: int):
    ...
    for q in questions:
        ...
        q_html_parts.append(f"<div style='margin:16px 0;padding:12px;border:1px solid #ddd;'><p><strong>Q{q.id}.</strong> {escape(getattr(q, f'question_text_{medium_key}'))}</p>{'' if (is_tap_select_image_question(q) or is_tap_correct_image_question(q) or is_drag_drop_group_container_question(q) or is_image_analysis_worksheet_question(q)) else image_html}{answer_html}</div>")
