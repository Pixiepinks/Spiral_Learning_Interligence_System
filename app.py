...
def student_dashboard():
    ...
    for index, (module, _, subject) in enumerate(modules):
        ...
        continue_cards.append(
            f"<div class='swiper-slide'><article class='continue-module-card'><img src='{escape(module_image)}' alt='{escape(module_name)}' class='continue-module-cover' loading='lazy'><div class='continue-module-body'><h4 class='continue-module-title'>{escape(module_name)}</h4><p class='continue-module-subject'>{escape(subject_name)}</p><div class='continue-module-progress'><span>{progress_value}%</span><button type='button' class='continue-module-play' aria-label='{escape(continue_label)}'>▶</button></div><div class='continue-module-progress-bar'><span class='continue-module-progress-fill' style='width:{progress_value}%;'></span></div></div></article></div>"
        )
    continue_empty = "මෙම ශ්‍රේණියට මොඩියුල නොමැත." if language == "si" else "No modules available for your grade."
    continue_html = "".join(continue_cards) or f"<div class='swiper-slide'><div class='card' style='padding:16px;'>{continue_empty}</div></div>"
...
(function () {{
  ...
  swiperScript.onload = function () {{
    const continueEl = document.getElementById("continueLearningSwiper");
    if (!continueEl || !window.Swiper) return;
    new window.Swiper(continueEl, {{
      effect: "coverflow",
      grabCursor: true,
      centeredSlides: true,
      loop: true,
      ...
      coverflowEffect: {{ rotate: 10, stretch: 0, depth: 90, modifier: 1.0, slideShadows: false, scale: 0.96 }},
      autoplay: {{ delay: 3500, disableOnInteraction: false }},
      navigation: {{ nextEl: "#continueNext", prevEl: "#continuePrev" }},
      pagination: {{ el: "#continuePagination", clickable: true }},
      breakpoints: {{
        0: {{ coverflowEffect: {{ rotate: 8, stretch: 0, depth: 70, modifier: 1, slideShadows: false, scale: 0.95 }} }},
        768: {{ coverflowEffect: {{ rotate: 10, stretch: 0, depth: 90, modifier: 1.0, slideShadows: false, scale: 0.96 }} }},
      }},
    }});
  }};
